...
export default function TableOfContent({ url, headings }: { url: string, headings: Heading[] }) {
  ...
  return (
    <div class={`px-3 py-1 fixed bottom-5 left-1/2 -translate-x-1/2 z-20 w-[250px] border-2 rounded cursor-pointer bg-white`} onClick={toggleToc}>
      ...
      <Show when={tocOpen()}>
        <ul class="toc list-disc flex flex-col gap-1 mt-2" use:clickOutside={toggleToc}>
          <For each={headings}>
            {
              (heading) => (
                <li
